...
\be
\label{eq:nbc}
\int_{A_\phi^{\rm HE}}^{A_\phi^{\rm EE}} \frac{B^2-E^2}{B^2+E^2} dA_\phi \Bigg/ \left(A_\phi^{\rm EE}- A_\phi^{\rm HE}\right)  < 10^{-3}.
\ee
...
and we choose $B^2+E^2$ to be the energy density measured by the zero-angular-momentum-observers.
Specifically,
\be
\begin{aligned}
...
recent high-resolution simulations performed by East and Yang \cite{East2018} (EY Sims). In Figure \ref{fig:Edot}, the solid
lines are curves fitted by polynomials

We find the angular velocity functions
in two studies show a good agreement, while the value of our magnetic flux $\AEE$ is lower by $\sim 5\%$, which gives
to $\sim 10\%$ lower energy extraction rates.
...
\caption{\label{fig:Edot} Comparison of the energy extraction rate $\dot E(\WH)$ derived from our
numerical solutions and from the simulations \cite{East2018},
where the former is lower than the latter one by $~10\%$.}
\end{figure}
