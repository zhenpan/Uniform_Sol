...
% From the expression of the LS function (\ref{eq:ls}), it is straightforward to see
% $ r_+ \leq r_{\rm LS}|_{\mu=0} \leq 2$ for $0 \leq \Omega \leq \Omega_{\rm H}$.
%  Due to the frame-dragging effect, $\Omega$
% must be non-zero inside the ergosphere.
%Therefore the only possibility is  $r_{\rm LS}|_{\mu=0} = 2$,
