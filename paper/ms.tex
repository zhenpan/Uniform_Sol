...
To avoid possible numerical difficulty, we use following prescription
\be
\label{eq:nbc}
...
we point out that from the radiation condition (\ref{eq:rad}) and the marginally
force-free boundary condition (\ref{eq:bc3})
themselves contains rich information about the BH magnetosphere structure.
...
(2) there is no current sheet within the magnetosphere except the equatorial current
...
both of which carry electric current and therefore Poynting flux. With the guidance of the qualitative
features above, we are to numerically solve the GS equation and quantify these features.
...
guaranteeing the boundary condition (\ref{eq:bc3}), which itself cannot be directly
used in computation.
...
\ee
where $\zeta_1$ is an
empirical step size. For each new $A_{\phi,\mu}$, we repeat Step 2
until $\Am(\mu = 0, r_+ \leq r\leq 2)$ converges, i.e. the condition $2\Omega\Ap(2\Omega\Ap)' = II'$ is
...
2\Ap\Omega_{\rm new}  = I|_{0 < \Ap < \AHE},
 \ee
 and the second part can be realized by iterative corrections
 \be
...
and in the upper panel of Figure \ref{fig:omega}, we show the angular velocity functions $\Omega(A_\phi)$ for different BH spins.
...
\caption{\label{fig:omega} Upper panel: the angular velocity functions $\Omega(\Ap)$ for different BH spins.
Lower Panel: comparison of our numerical results (solid lines) with the simulation results in \cite{East2018} (dashed lines).}
\end{figure}


\begin{figure}
\includegraphics[scale=0.6]{f3}
\caption{\label{fig:Edot} Comparison of the energy extraction rate $\dot E(\WH)$ obtained from our
numerical solutions and from the high-resolution force-free simulations \cite{East2018},
where the former is lower than the latter by $\sim 10\%$ for large spins.}
\end{figure}

With the angular velocity $\Omega(\Ap)$ available, the energy extraction rate from the BH is known as
\be
\label{eq:Edot}
\dot E = 4\pi \int_0^{\AEE} \Omega \times I \ d \Ap.
\ee
In the limit of slow rotation, analytic solution is known as \cite{Pan2014, East2018},
\be
    \Omega = \WH\frac{\sqrt{1-\psi}} {1+\sqrt{1-\psi}},
\ee
where $\psi = \Ap/(2B_0M^2)$, and the energy extraction rate is
\be
\label{eq:lowEdot}
\dot E = 128\pi \left(\frac{17}{24}-\ln 2\right) B_0^2 M^4 \WH^2.
\ee

In the lower panel of Figure  \ref{fig:omega} and in Figure \ref{fig:Edot}, we compare our numerical results with
recent high-resolution simulations done by East and Yang \cite{East2018}. In Figure \ref{fig:Edot},
the points are taken from either the simulations or our numerical solutions, while the solid lines are
corresponding polynomial fitting curves which require to approach Eq.(\ref{eq:lowEdot}) for small spins and to be flat for extremal spins.
We find the angular velocity functions obtained from two different approaches yield a good agreement (Figure \ref{fig:omega}),
while the energy extraction rate $\dot E$ is $\sim 10\%$ lower than the corresponding simulation results, which is traced to
$\sim 5\%$ smaller magnetic flux $\AEE$ in our numerical solutions.
...
being strictly vertical. Due to the same topology, it is reasonable to expect the marginally force-free boundary
...
\subsection{Near-horizon field lines}
...
horizon" based on two
However, simulations shows that there exists an equatorial current sheet within the ergosphere, where
